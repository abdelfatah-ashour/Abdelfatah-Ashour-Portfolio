...
export default function Project({ project }: InferGetStaticPropsType<typeof getStaticProps>) {
  const { title, description, shortDescription, banner, dimensions, stack, deployment, screenshots, subProjects } =
    project;
    ...
    (screenshot: string) => {
      ...
      return (
        <div
          ...
          style={style}
        >
          <Image loading="eager" src={screenshot} height={height} width={width} objectFit="cover" alt="" />
        </div>
      );
    },
    [height, width]
  ...
  return (
    <>
      <PageSEO title={title} description={shortDescription || description} imageUrl={banner} />
      <H1 className="lg:text-5x mb-4 text-3xl font-bold dark:text-white">{title}</H1>
